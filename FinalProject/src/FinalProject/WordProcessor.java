...
public class WordProcessor {
	public static void main(String[] args) throws Exception {
		...
		// change the file location to where the test.txt is on your desktop, we will
		// implement a system that works better
		...
		// variables holding the word leftover from previous line and the count of
		// characters in it
		String leftoverWord = "";
		int leftoverCount = 0;
		int leftOverChars = 0;
		...
		do {
			...
			if (currentChar.equals("-")) {
				...
				case ("n"):
					...
					int temp1 = Integer.parseInt(tempSub); // we should probably throw error if there is no number
															// following the command n

					if (temp1 <= 100) {
						lineLength = temp1;
					} else // we probably need to change the response to the error to something more
							// appropriate
					{
						errCount++;
				...
				case ("w"):
						...
						// appropriate
					{
						errCount++;
						errLog.write(
								"Error on line " + lineReadInCount + " : Wrapping command must be followed by + or -.");
						errLog.newLine();
					}
					break;
				case ("s"):
				...
				case ("p"):
					...
					int temp3 = Integer.parseInt(tempSub2); // we should probably throw error if there is no number
															// following the command n
					// checking if the number of indents exceeds the current number of characters
					// per line
					if (temp3 < lineLength)
						paragraph = temp3;
					else// we probably need to change the response to the error to something more
						// appropriate
					{
						errCount++;
						errLog.write(
								"Error on line " + lineReadInCount + " : Number of indents exceeds the line length.");
						errLog.newLine();
					}
					break;
				case ("b"):
				...
				case ("a"):
						...
						// appropriate
					{
						errCount++;
						errLog.write(
								"Error on line " + lineReadInCount + " : You can only choose between 1 or 2 columns.");
						errLog.newLine();
					}
					break;
				}
			...
			// if columns are not set to 2 columns
				...
				for (int i = 0; i < currentLine.length() - 1; i++) {
					...
					// if title is on and the line length is longer than count give an errr but
					// still format the appropriate length of it

					// else if we are starting paragraph

					// if it is left justified and number of characters is 0 we print the number of
					// indents and then the current character
					if (justified == 'l' && paragraph != 0 && startParagraph) {
						errCount++;
						errLog.write("Error on line " + lineReadInCount
								+ " : Paragraph indent can only be used with left justification");
						errLog.newLine();
					}
					...
					if (!wrapping) {
						...
						while (currentCharCount < lineLength && i < currentLine.length()) {
							...
							// if we reach the end of the word or in other words space add the word to the
							// output line and end that line as well as adding the count for the space
							else {
						...
						// if it is right justified add the number of leftover spaces in the beginning
						// of the output line
						if (justified == 'r') {
						...
						// if it is center justified divide the number of leftover count in 2 and add
						// half in the beginning
						if (justified == 'c') {
							System.out.println("Justification is: " + justified);
						...
						// when we reach the maximum character length set for the output we set the char
						// count to leftover char count
						// and we print the line to file using println and we reset the current output
						// line to the word we have left from before
						...
						if (title && (currentLine.length() > lineLength)) {
							errCount++;
							errLog.write("Error on line " + lineReadInCount
									+ " : Title has to fit on one line, with the length specified by user");
							System.out.println("line 186");
							System.out.println(currentLine.length());
							System.out.println("Error on line " + lineReadInCount
									+ " : Title has to fit on one line, with the length specified by user");
							errLog.newLine();
						} else if (title) {
					...
					// if wrapping is on
						...
						// while the count of characters is less then line length keep constructing the
						// output line
						...
						leftOverLines = lineLength - currentLine.length();
						while (currentCharCount < lineLength && i < currentLine.length() - 1) {
							currentOutputLine = currentOutputLine + currentChar;
							if (currentChar.equals(" ")) {
								numberOfWords++;
						...
						while (currentCharCount < lineLength && i <= currentLine.length() - 1) {
							int numSpaces = lineLength - currentOutputLine.length();

							if (justified == 'e') {

								while (numSpaces > 0) {

									for (int p = 0; p < currentOutputLine.length(); p++) {

										if (currentOutputLine.charAt(p) == ' '
												&& currentOutputLine.charAt(p - 1) != ' ') {

											StringBuilder addSpace = new StringBuilder(currentOutputLine);
											
											addSpace.insert(p, ' ');
											
											currentOutputLine = addSpace.toString();
							...
							i++;
							// and we fetch next character of the line
							currentChar = Character.toString(currentLine.charAt(i - 1));
						}
						numberOfWords = 0;
					...
					i--;
					System.out.println("Starting the next output line with the i=" + i
							+ " and the current count of characters = " + currentCharCount);
				}
